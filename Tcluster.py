    ...
    for s in range(len(siteid)):
        if siteid[s] in metlist:
            ...
            if sum(M_datevalid[s, :]) > 30:  # more than 30 valid days sitem_id[s]=='BKLN': #s in range(len(sitem_id)):##
                ...
                # kmean cluster
                if clusterm == 'kmeans':
                    kmeans = KMeans(n_clusters=clustern, init='k-means++', max_iter=300)#, n_init=10
                    if normm==1:
                ...
                for g in range(clustern):
                    ...
                    plt.plot(mett, color=cols[g * 2 + 1],linewidth=2, label= Cluname[g]+' ' + str(len(np.where(M_y[s, :] == g_ind[g])[0])))
                    ...
                    plt.xlabel('EST', fontsize=tkfont)
                    plt.ylabel('T (℃)', fontsize=tkfont)
                    plt.legend(frameon=False, fontsize=tkfont,loc='best')
                    plt.tick_params(labelsize=tkfont)
                    plt.ylim([13, 35])
                    plt.yticks(np.arange(15, 35, 3))
                    plt.xlim([0, 23])
                    plt.xticks(np.arange(0, 24, 4),
                               ['00:00', '04:00', '08:00', '12:00', '16:00', '20:00'])

                fig.savefig(outpath+'/T' + sitename[s] + siteid[s]  +'ClusterMed.png', dpi=600)
                fig.savefig(outpath + '/T' + sitename[s] + siteid[s] + 'ClusterMed.pdf', dpi=600)
                plt.close()
