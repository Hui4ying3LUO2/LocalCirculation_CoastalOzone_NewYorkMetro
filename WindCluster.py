            ...
            if siteid[s] in metlist:
                ...
                if sum(M_datevalid[s, :]) > 30:  # more than 30 valid days sitem_id[s]=='BKLN': #s in range(len(sitem_id)):##
                    ...
                    for g in range(clustern):
                        ...
                        ax2 = fig.add_subplot(2, clustern, g+1, projection='windrose')
                        ...
                        ax2.set_title( Cluname[g]+' '+str(WSg.shape[1])+'', fontsize=15, fontweight='bold')
                        ax2.set_yticks(np.arange(0, 40, step=10))
                        ax2.set_yticklabels(np.arange(0, 40, step=10), fontsize=10)
                        ax2 = fig.add_subplot(2,clustern,g+1+clustern,projection='windrose')
                    ...
                    fig.savefig(outpath+'/' + siteid[s] +clusterm + '_norm'+str(normm) + '_'+'_'+str(para_n)+'_'+str(clustern)+ 'ClusterWindrose.png', dpi=300, bbox_inches='tight')
                    plt.close()
